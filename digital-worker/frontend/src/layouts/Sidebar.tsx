...
const menuItems = [
  ...
  },
  {
    ...
    label: '月账管理',
    children: [
      { key: '/monthly/monitor', icon: <BarChartOutlined />, label: '进度监控' },
      { key: '/monthly/task-orchestration', icon: <DeploymentUnitOutlined />, label: '任务编排' },
      { key: '/monthly/daily-report', icon: <FileTextOutlined />, label: '出账日报' },
      { key: '/monthly/report-publish', icon: <SendOutlined />, label: '报表发布' },
      { key: '/monthly/billing-progress', icon: <BarChartOutlined />, label: '月账进度' },
      { key: '/monthly/billing-task-config', icon: <ToolOutlined />, label: '任务配置' },
      { key: '/monthly/ledger-display', icon: <AppstoreOutlined />, label: '月账展示' },

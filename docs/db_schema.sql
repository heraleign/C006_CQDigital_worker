...
CREATE TABLE ma_task_monitor (
    ...
    stage_name VARCHAR(200) COMMENT '阶段名称',
    milestone_code VARCHAR(50) COMMENT '里程碑编码',
    milestone_name VARCHAR(200) COMMENT '里程碑名称',
    plan_code VARCHAR(50) COMMENT '作业计划编码',
    plan_name VARCHAR(200) COMMENT '作业计划名称',
    task_code VARCHAR(100) NOT NULL COMMENT '任务编码',
    task_name VARCHAR(200) COMMENT '任务名称',
    task_type VARCHAR(50) COMMENT '任务类型: TDP_TASK/PUBLISH_MSG/MANUAL_OP/SQL_SCRIPT',
    ...
    status VARCHAR(20) DEFAULT 'pending' COMMENT '状态: pending/running/completed/failed/skipped/paused/manual_skipped',
